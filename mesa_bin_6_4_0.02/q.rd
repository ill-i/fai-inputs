...
<table id="main" onDisk="True" adql="True">
	...
	<regSuite title="MESA Grid Verification">
		<regTest title="Test TAP access by physical period">
			<url parSet="TAP"
					 QUERY="SELECT component, m1, period FROM mesa_bin_6_4_002.main WHERE period BETWEEN 0.899 AND 0.901"
					 >/tap/sync</url>
			<code>
				...
				# Проверяем период с допуском (важно для float!)
				p_val = float(row["period"])
				self.assertTrue(abs(p_val - 0.9) &lt; 0.001, f"Period {p_val} is not 0.9")
			</code>
		</regTest>
	</regSuite>
