...
<!-- this stylesheet is based on what's given in

https://volute.g-vo.org/svn/trunk/projects/registry/dois

and should follow changes made there.  It adds:

(a) identifier is taken from vr:altIdentifier and text manipulation
(b) it adds d:altIdentifiers for the reference URL and WebBrowser
    interfaces.

This (and in particular the validity violation that we leave out
identifier if there's no upstream DOI) is done by agreement with
b2find, who use this to harvest VO records.
-->
...
<xsl:stylesheet
    ...
    <xsl:template match="altIdentifier">
      ...
      <xsl:if test="starts-with(., 'doi:')">
        <d:identifier identifierType="DOI"><xsl:value-of
          select="substring-after(., 'doi:')"/></d:identifier>
      </xsl:if>
    </xsl:template>

    <xsl:template match="interface">
      <!-- only WebBrowser-typed interfaces are relevant for b2find.
        We make alternateIndentifiers from them (by agreement with b2find).
        -->
      <xsl:if test="@xsi:type='vr:WebBrowser'">
        <d:alternateIdentifier alternateIdentifierType="browser URL"
          ><xsl:value-of select="accessURL[1]"/></d:alternateIdentifier>
      </xsl:if>
    </xsl:template>

    <xsl:template match="referenceURL">
      <!-- we make an alternateIndentifier from reference URL by 
        agreement with b2find.  -->
      <d:alternateIdentifier alternateIdentifierType="reference URL"
        ><xsl:value-of select="."/></d:alternateIdentifier>
    </xsl:template>
