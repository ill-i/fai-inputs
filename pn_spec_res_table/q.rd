<resource schema="pn_spec_res_table" resdir=".">
	...
	<table id="main" onDisk="True" adql="True">
		...
		<column name="Ne_SII_err"
				type="double precision"
				unit="cm**-3"
				ucd="stat.error;phys.density.electron"
		...
		<column name="HeII_4685_err"
				type="double precision"
				unit="erg cm**-2 s**-1"
				ucd="stat.error;phot.flux;em.line.HeII"
		...
		<column name="NII_6583_err"
				type="double precision"
				unit="erg cm**-2 s**-1"
				ucd="stat.error;phot.flux;em.line.NII"
		...
		<column name="HeI_6678"
 		...
 		<column name="HeI_6678_err"
				type="double precision"
				unit="erg cm**-2 s**-1"
				ucd="stat.error;phot.flux;em.line.HeI"
		...
		<column name="SII_6717_err"
				type="double precision"
				unit="erg cm**-2 s**-1"
				ucd="stat.error;phot.flux;em.line.SII"
		...
		<column name="ArIII_7136_err"
				type="double precision"
				unit="erg cm**-2 s**-1"
				ucd="stat.error;phot.flux;em.line.ArIII"
		...
		<column name="OI_7324_err"
				type="double precision"
				unit="erg cm**-2 s**-1"
				ucd="stat.error;phot.flux;em.line.OI"
	...
	<regSuite title="pn_spec_res_table regression">
		...
		<regTest title="Table returns at least one row with valid data">
			<url parSet="TAP"
				QUERY="SELECT object, \'F(H_beta)\', T_HI, age_year FROM main LIMIT 1">
			</url>
			<code>
				row = self.getFirstVOTableRow()
				self.assertTrue(row["object"])	# Проверяем, что есть идентификатор
				self.assertGreater(float(row["F(H_beta)"]), 0)	# Проверяем, что поток положительный
		...
		<regTest title="Object search by name returns correct row">
			<url parSet="TAP"
				QUERY="SELECT object FROM main WHERE object = 'M1-6'"
				>/tap/sync</url>
			<code>
				rows = self.getVOTableRows()
				self.assertTrue(any(row["object"] == "M1-6" for row in rows))
			</code>
		</regTest>
		...
		<regTest title="Range search for extinction_C_Hbeta">
			<url parSet="TAP"
				QUERY="SELECT extinction_C_Hbeta FROM main WHERE extinction_C_Hbeta BETWEEN 0.0 AND 2.0"
				>/tap/sync</url>
			<code>
		...
		<regTest title="Exact match for Diam_arcsec">
			<url parSet="TAP"
				QUERY="SELECT Diam_arcsec FROM main WHERE Diam_arcsec = '5'"
				>/tap/sync</url>
			<code>
				for row in self.getVOTableRows():
		...
		<regTest title="Range search for Dist_kpc">
			<url parSet="TAP"
				QUERY="SELECT Dist_kpc FROM main WHERE Dist_kpc BETWEEN 1 AND 10"
				>/tap/sync</url>
			<code>
				for row in self.getVOTableRows():
		...
		<regTest title="Range search for Ne_SII">
			<url parSet="TAP"
				QUERY="SELECT Ne_SII FROM main WHERE Ne_SII BETWEEN 1E2 AND 1E5"
				>/tap/sync</url>
			<code>
				for row in self.getVOTableRows():
		...
		<regTest title="Range search for T_HI">
			<url parSet="TAP"
				QUERY="SELECT T_HI FROM main WHERE T_HI BETWEEN 2E4 AND 2E5"
				>/tap/sync</url>
			<code>
				for row in self.getVOTableRows():
					val = float(row["T_HI"])
		...
		<regTest title="Range search for age_year">
			<url parSet="TAP"
				QUERY="SELECT age_year FROM main WHERE age_year BETWEEN 100 AND 10000"
				>/tap/sync</url>
			<code>
				for row in self.getVOTableRows():
